...
use crate::igdb::apicalypse_builder::ApicalypseBuilder;
use crate::igdb::client::IGDBApiError::AuthError;
use microjson::JSONParsingError;
use prost::DecodeError;
use thiserror::Error;
use crate::igdb::igdb::Count;
...
impl Client {
    ...
    async fn check_access_token(&mut self) -> Result<(), IGDBApiError> {
        if self.client_access_token.is_empty() {
            ...
            self.client_access_token = JSONValue::load(resp.await.unwrap().as_str())
